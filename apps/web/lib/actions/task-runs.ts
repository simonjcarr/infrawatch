...
export async function triggerCustomScriptRun(
  ...
  return createTaskRun(orgId, user.id, 'host', hostId, 'custom_script', config, 1, [hostId], [])
}
...
export async function triggerGroupCustomScriptRun(
  ...
  const config: CustomScriptTaskConfig = { script, interpreter, ...(timeoutSeconds ? { timeout_seconds: timeoutSeconds } : {}) }
  return createTaskRun(orgId, user.id, 'group', groupId, 'custom_script', config, maxParallel, hostIds, [])
}
...
export async function triggerServiceAction(
  ...
  return createTaskRun(orgId, user.id, 'host', hostId, 'service', config, 1, [hostId], [])
}
...
> {
  ...
  const result = await createTaskRun(
    orgId, user.id, 'group', groupId, 'service', config, maxParallel, pendingHostIds, skipHosts,
  )
...
export async function triggerAgentUninstall(
  ...
  return createTaskRun(orgId, user.id, 'host', hostId, 'agent_uninstall', config, 1, [hostId], [])
}
...
export async function triggerPatchRun(
  ...
  return createTaskRun(orgId, user.id, 'host', hostId, 'patch', config, 1, [hostId], [])
}
