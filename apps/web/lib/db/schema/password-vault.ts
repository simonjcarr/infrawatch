...
export const PASSWORD_VAULT_ROTATION_REASONS = ['initial', 'membership_revoked', 'manual_rewrap'] as const
...
export const passwordVaultMembers = pgTable(
  'password_vault_members',
  {
    ...
    userId: text('user_id')
      .notNull()
      .references(() => users.id, { onDelete: 'cascade' }),
    role: text('role').$type<PasswordVaultMemberRole>().notNull(),
    wrappedVaultKeyEnvelope: jsonb('wrapped_vault_key_envelope')
      .$type<PasswordVaultEnvelope>()
      .notNull(),
  ...
  (t) => [
    ...
    index('password_vault_members_user_idx').on(t.userId, t.revokedAt),
  ],
)
...
export const passwordVaultEntries = pgTable(
  ...
  (t) => [
    index('password_vault_entries_org_vault_updated_idx').on(t.organisationId, t.vaultId, t.updatedAt),
    index('password_vault_entries_vault_deleted_idx').on(t.vaultId, t.deletedAt),
  ],
)
