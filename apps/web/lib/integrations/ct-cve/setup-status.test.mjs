...
test('summarises DB-backed CT-CVE connector settings without exposing token identifiers or secrets', async () => {
    ...
    settingsRepository: {
      async getSummary(instanceId) {
        ...
        return {
          ...
          inventoryTokenId: 'ctops_inventory_instance_1',
          ctCveTokenId: 'ctcve_findings_instance_1',
  ...
  assert.equal(JSON.stringify(overview).includes('ctcve_findings_instance_1'), false)
  assert.equal(JSON.stringify(overview).includes('ctops_inventory_instance_1'), false)
})
