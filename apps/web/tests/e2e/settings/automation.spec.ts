...
test('admin can enable Ansible automation and create an SSH credential profile', async ({ authenticatedPage: page }) => {
  ...
  await expect(page.getByText(/Pair the Ansible API connection/)).toBeVisible()
  ...
  await expect(page.getByText(/Keep the generated token file on persistent storage/)).toBeVisible()
  await expect(page.getByTestId('ansible-pairing-url')).toBeVisible()
  await expect(page.getByTestId('ansible-pairing-username')).toBeVisible()
  await expect(page.getByTestId('ansible-pairing-password')).toBeVisible()
