...
check_env() {
  ...
  if [ -z "${ANSIBLE_API_PAIRING_USERNAME:-}" ]; then
    upsert_env_var "ANSIBLE_API_PAIRING_USERNAME" "ctops"
    echo "Set ANSIBLE_API_PAIRING_USERNAME in .env."
  fi

  if [ -z "${ANSIBLE_API_PAIRING_PASSWORD:-}" ]; then
    require_openssl "generate ANSIBLE_API_PAIRING_PASSWORD on first run"
    GENERATED_ANSIBLE_PAIRING_PASS=$(openssl rand -base64 24)
    upsert_env_var "ANSIBLE_API_PAIRING_PASSWORD" "$GENERATED_ANSIBLE_PAIRING_PASS"
    echo "Generated ANSIBLE_API_PAIRING_PASSWORD and wrote it to .env."
  fi
