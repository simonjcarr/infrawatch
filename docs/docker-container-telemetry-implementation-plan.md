...
| Implement retention sweeper | Codex | review | ingest sweeper or web maintenance job, tests | Global and host settings | Rows are deleted according to each host's effective retention. | TBD | PR pending. Ingest sweeper deletes metrics by host-effective retention and purges Docker telemetry batch records after 7 days. |
