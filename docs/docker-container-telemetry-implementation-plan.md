...
| Task | Owner | Status | Files / Areas | Dependencies | Acceptance Criteria | PR | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Implement agent sampler and buffer | Codex | review | agent Docker sampler/buffer, tests | Phase 2 inventory | Agent samples every 2 seconds by default, stores bounded buffered samples, and records dropped sample counts when full. | [#1359](https://github.com/carrtech-dev/ct-ops/pull/1359) | PR #1359 implements the in-memory sampler/buffer. Upload remains a separate task. |
