...
| Task | Owner | Status | Files / Areas | Dependencies | Acceptance Criteria | PR | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Implement agent sampler and buffer | Codex | done | agent Docker sampler/buffer, tests | Phase 2 inventory | Agent samples every 2 seconds by default, stores bounded buffered samples, and records dropped sample counts when full. | [#1359](https://github.com/carrtech-dev/ct-ops/pull/1359) | Merged in #1359 and released as agent v0.37.0 in #1360. Upload remains a separate task. |
| Implement batched telemetry upload | Codex | done | protobuf RPC/heartbeat integration, agent sender, ingest handler | Sampler/buffer | Agent flushes batches on normal interval; ingest enforces max payload size, timestamp bounds, and idempotency. | [#1362](https://github.com/carrtech-dev/ct-ops/pull/1362) | Merged in #1362 and released in #1363 as agent v0.38.0, ingest v0.12.0, web v0.141.0, and bundle v0.7.0. Upload sends inventory before samples, uses batch ids/sequences, and enforces payload/timestamp limits. |
| Store container metric samples | Codex | done | `docker_container_metrics` schema, ingest DB queries, tests | Batched upload | Metrics are stored with instance, host, container, recorded_at, CPU, memory, network, block I/O, pids, and restart count. | [#1362](https://github.com/carrtech-dev/ct-ops/pull/1362) | Merged in #1362 and released in #1363. Added metric and idempotency tables, ingest normalization, and composite query indexes. |
| Add per-container metric charts | Codex | done | web actions, chart components, e2e/component tests | Stored metrics | UI shows CPU, memory, network, block I/O, and pids over time with avg and max where bucketed. | [#1365](https://github.com/carrtech-dev/ct-ops/pull/1365) | Merged in #1365 and released in #1366 as web v0.142.0 and bundle v0.8.0. Max line/area preserves spike visibility. |
...
| Add host retention override | Codex | done | host settings UI/actions/schema | Global setting | Host settings can set or clear a local Docker retention override. | [#1372](https://github.com/carrtech-dev/ct-ops/pull/1372) | Merged in #1372. Implements nullable `HostMetadata.dockerSettings.retentionDaysOverride` with inherited/default/effective display. |
| Implement retention sweeper | Codex | done | ingest sweeper or web maintenance job, tests | Global and host settings | Rows are deleted according to each host's effective retention. | [#1375](https://github.com/carrtech-dev/ct-ops/pull/1375) | Merged in #1375 and released in #1376 as ingest v0.13.0 and bundle v0.11.0. Ingest sweeper deletes metrics by host-effective retention and purges Docker telemetry batch records after 7 days. |
| Document retention behavior | Codex | done | docs site, feature docs | Retention implementation | Docs explain defaults, overrides, storage impact, and cleanup timing. | [#1378](https://github.com/carrtech-dev/ct-ops/pull/1378) | Merged in #1378. Added operator-facing Docker telemetry docs, linked ingest retention behavior, and included sizing caveats for large fleets. |
...
| Add alert rules for containers | Codex | done | alert schema/evaluator/UI | Phase 3 metrics | Alerts cover restart loop, memory near limit, sustained CPU, container missing, and high network I/O. | [#1385](https://github.com/carrtech-dev/ct-ops/pull/1385) | Merged in #1385 and released in #1386 as ingest v0.15.0, web v0.147.0, and bundle v0.14.0. Avoids one-sample noise with sample/window thresholds for metric-derived rules. |
