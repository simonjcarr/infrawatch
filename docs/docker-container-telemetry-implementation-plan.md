...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Define Docker telemetry protobuf contract | Codex | review | `proto/agent/v1/*.proto`, generated Go protobufs | None | Messages represent Docker status, container inventory, metric samples, batch ids, dropped sample counts, and payload limits. Existing agents remain compatible. | TBD | Added heartbeat Docker status/config and dedicated Docker telemetry upload RPC; validation passed locally. |
