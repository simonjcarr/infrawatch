...
| 5 | Add vault schema and migration | 4 | Complete | automation/work-on-password-manager | [#887](https://github.com/carrtech-dev/ct-ops/pull/887) | Added `apps/web/lib/db/schema/password-vault.ts`, exported it from the schema index, generated `0056_ordinary_colossus.sql`, added E2E truncate coverage, and backfilled the missing `0055_snapshot.json` so future Drizzle diffs stay clean. |
...
| 2026-05-04 | automation/work-on-password-manager | 5 | Complete | [#887](https://github.com/carrtech-dev/ct-ops/pull/887) | `node --experimental-strip-types --test lib/db/schema/password-vault.contract.test.mjs lib/db/schema/password-vault.schema.test.mjs`; `pnpm --filter web db:generate`; `pnpm --filter web db:migrate`; `pnpm --filter web db:validate`; `pnpm --filter web exec eslint lib/db/schema/password-vault.ts lib/db/schema/password-vault.schema.test.mjs lib/db/schema/index.ts tests/e2e/fixtures/db.ts`; `pnpm --filter web build`; `git diff --check` | Added the Password Vault Drizzle schema and generated migration, updated E2E truncation coverage, and backfilled the missing `0055_snapshot.json` so Drizzle no longer duplicates the earlier licence-verifier migration on future generates. |
